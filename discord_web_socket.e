...
inherit
	WEB_SOCKET_CLIENT
		redefine
			receive_handshake, analyze_request_message
		end
...
feature {NONE} -- Initialization
	...
	make(a_url:READABLE_STRING_8)
			-- Initialization of `Current' connecting to `a_url'
		do
			create open_actions
			...
			create implementation.make (create {WEB_SOCKET_NULL_CLIENT}, a_url)	-- Useless (for Void-Safety)
		end
...
feature -- Events API
	...
	close_actions:ACTION_SEQUENCE[TUPLE[code: INTEGER; reason: STRING]]
			-- Launched when the connection has been closed. The reason
			-- can be checked with `code' or with a human readable `reason'.
...
feature {NONE} -- Implementation
	...
	analyze_request_message
			...
			-- Must redefine it because the Discord server use some lowercase key but Eiffel Web Socket library
			-- does not support it (ToDo: Pull request).
		local
